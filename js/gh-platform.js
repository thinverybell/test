(function () {
  ...
  function navHTML(active) {
    // Prefer unified sidebar (same groups + role rules as homepage)
    if (window.GiaHuySidebar && typeof window.GiaHuySidebar.renderPlatformHTML === 'function') {
      return window.GiaHuySidebar.renderPlatformHTML(active);
    }
    const links=[
      ['index.html','cil-home','Trang chủ','home'],['plugins.html','cil-book','Bài giảng','plugins'],['config.html','cil-notes','Giáo án','config'],['mods.html','cil-task','Bài tập','mods'],['assets.html','cil-education','Học liệu','assets'],['tools.html','cil-settings','Công cụ','tools'],['resources.html','cil-folder-open','Tài nguyên','resources'],['guide.html','cil-lightbulb','Hướng dẫn','guide'],['flashcards.html','cil-layers','Flashcard','flashcards'],['quiz.html','cil-list-numbered','Quiz','quiz'],['videos.html','cil-video','Video','videos'],['games.html','cil-videogame','Game Center','games'],['my-library.html','cil-heart','Kho cá nhân','library'],['qna.html','cil-speech','Hỏi đáp','qna'],['settings.html','cil-cog','Cài đặt','settings']
    ];
    return links.map(([href,ico,label,key])=>`<a class="gh-side-link side-item ${active===key?'active':''}" href="${href}"><span class="side-icon">${ICON(ico)}</span><span>${label}</span></a>`).join('');
  }

  function pageShell({active,title,desc,kicker='THẦY GIA HUY'}) {
    document.body.innerHTML=`<div class="gh-app"><header class="gh-header topbar"><a class="gh-brand brand" href="index.html"><img src="assets/logo-giahuy.png" alt="Thầy Gia Huy" class="brand-logo"></a><nav class="gh-header-nav topnav"><a href="index.html" class="${active==='home'?'active':''}">${ICON('cil-home')} Trang chủ</a><a href="plugins.html" class="${active==='plugins'?'active':''}">${ICON('cil-book')} Bài giảng</a><a href="config.html" class="${active==='config'?'active':''}">${ICON('cil-notes')} Giáo án</a><a href="mods.html" class="${active==='mods'?'active':''}">${ICON('cil-task')} Bài tập</a><a href="assets.html" class="${active==='assets'?'active':''}">${ICON('cil-education')} Học liệu</a><a href="flashcards.html" class="${active==='flashcards'?'active':''}">${ICON('cil-layers')} Flashcard</a><a href="quiz.html" class="${active==='quiz'?'active':''}">${ICON('cil-list-numbered')} Quiz</a><a href="games.html" class="${active==='games'?'active':''}">${ICON('cil-videogame')} Game</a></nav><div class="gh-header-actions top-actions"><label class="gh-header-search search-pill">${ICON('cil-search')}<input id="globalSearch" placeholder="Tìm kiếm..." autocomplete="off"></label><a class="gh-icon-btn" href="notifications.html" title="Thông báo">${ICON('cil-bell')}</a><a class="gh-icon-btn" href="settings.html" title="Cài đặt">${ICON('cil-cog')}</a><a href="profile.html"><img class="gh-avatar" src="assets/images/avatar.gif" alt="Hồ sơ"></a></div></header><div class="gh-layout"><aside class="gh-sidebar"><div class="side-inner">${navHTML(active)}</div></aside><main class="gh-main"><div class="gh-container"><div class="gh-page-head"><div><div class="kicker">${esc(kicker)}</div><h1>${esc(title)}</h1><p>${esc(desc)}</p></div><div class="gh-actions"><a class="gh-btn" href="index.html">${ICON('cil-arrow-left')} Trang chủ</a></div></div><div id="pageRoot"></div><footer class="gh-footer"><div class="gh-footer-inner"><span>© ${new Date().getFullYear()} Thầy Gia Huy</span><span>Học tập trực quan · Thầy Gia Huy</span></div></footer></div></main></div><nav class="gh-bottom-nav"><a class="${active==='home'?'active':''}" href="index.html">${ICON('cil-home')}Trang chủ</a><a class="${active==='plugins'?'active':''}" href="plugins.html">${ICON('cil-book')}Bài giảng</a><a class="${active==='games'?'active':''}" href="games.html">${ICON('cil-videogame')}Game</a><a href="settings.html">${ICON('cil-menu')}Menu</a></nav></div>`;
    try{document.dispatchEvent(new CustomEvent('gh:platform-ready'));}catch(_){}
    if(window.GiaHuySidebar){try{window.GiaHuySidebar.apply();}catch(_){}}
