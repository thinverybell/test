...
function card(x){
  ...
  return `<article class="resource-card" id="resource-${esc(x.id)}">` +
    `<div class="card-top">` +
      `<span class="resource-icon">${iconFor(x)}</span>` +
...
form.querySelector('.dropzone').addEventListener('click',()=>input.click());input.addEventListener('change',()=>form.querySelector('.dropzone span').textContent=input.files.length?`${input.files.length} file đã chọn`:'Chưa chọn file');const dz=form.querySelector('.dropzone');['dragenter','dragover'].forEach(ev=>dz.addEventListener(ev,e=>{e.preventDefault();dz.classList.add('dragging')}));['dragleave','drop'].forEach(ev=>dz.addEventListener(ev,e=>{e.preventDefault();dz.classList.remove('dragging')}));dz.addEventListener('drop',e=>{const dt=new DataTransfer();[...e.dataTransfer.files].forEach(f=>dt.items.add(f));input.files=dt.files;input.dispatchEvent(new Event('change',{bubbles:true}))});form.addEventListener('submit',async e=>{e.preventDefault();const fs=[...input.files];if(!fs.length){notify('Chưa chọn file.','error');return}const fd=new FormData(form);const accType=fd.get('access')==='paid'?'paid':'free';if(!canUploadAccess(accType)){notify(accType==='paid'?'Không có quyền upload tài liệu Paid.':'Không có quyền upload tài liệu Free.','error');return}for(const f of fs){await add('files',{name:fs.length===1?(fd.get('name')||f.name.replace(/\.[^.]+$/,'')):f.name.replace(/\.[^.]+$/,''),cat:fd.get('cat'),desc:fd.get('desc'),fileName:f.name,mime:f.type,size:f.size,blob:f,created:Date.now(),builtin:false,featured:fd.get('featured')==='on',access:(fd.get('access')==='paid'?'paid':'free'),creatorName:(()=>{try{const s=JSON.parse(localStorage.getItem('giahuy-session')||'null');if(!s)return'Admin';if(s.role==='admin'){try{return JSON.parse(localStorage.getItem('giahuy-admin-profile')||'{}').username||s.username||'Quản trị viên'}catch{return s.username||'Quản trị viên'}}return s.username||'—'}catch{return'—'}})(),creatorRole:(()=>{try{const s=JSON.parse(localStorage.getItem('giahuy-session')||'null');if(!s)return'quản trị';return s.role==='teacher'?'giáo viên':s.role==='manager'?'quản lí':'quản trị'}catch{return'quản trị'}})()})}notify(`Đã upload ${fs.length} file.`);form.reset();renderAdmin('upload');renderCatalog();renderHome()})}
