(function(){
  ...
  const viStatus=s=>({verified:'ĐÃ XÁC MINH',pending:'ĐANG CHỜ',conflict:'XUNG ĐỘT',unverified:'CHƯA XÁC MINH',VALID:'HỢP LỆ',WARNING:'CẢNH BÁO',ERROR:'LỖI',success:'THÀNH CÔNG',error:'LỖI',warning:'CẢNH BÁO',info:'THÔNG TIN',READY:'SẴN SÀNG',EDITING:'ĐANG CHỈNH SỬA',VERIFYING:'ĐANG KIỂM TRA',COMPILING:'ĐANG BIÊN DỊCH',RUNNING:'ĐANG CHẠY',PAUSED:'TẠM DỪNG',FAULT:'CÓ LỖI',STOPPED:'ĐÃ DỪNG',RESETTING:'ĐANG ĐẶT LẠI',DISCONNECTED:'CHƯA KẾT NỐI'}[s]||s);
  const viCategory=s=>({All:'Tất cả',Board:'Bo mạch',Module:'Linh kiện',Sensor:'Cảm biến',Actuator:'Cơ cấu chấp hành',Prototyping:'Lắp mạch',Communication:'Giao tiếp',Display:'Hiển thị',Power:'Nguồn',Input:'Đầu vào',Output:'Đầu ra'}[s]||s);
  const viSubsystem=s=>({System:'Hệ thống','Board Registry':'Quản lý bo mạch',Wiring:'Kết nối dây',Compiler:'Trình biên dịch',Runtime:'Mô phỏng',Recovery:'Phục hồi',Persistence:'Lưu trữ',Asset:'Tài nguyên',Lesson:'Bài học'}[s]||s);
  const viSupport=s=>({'native':'TÍCH HỢP','composite':'TỔ HỢP','behavioral':'MÔ HÌNH HÀNH VI','visual-only':'CHỈ HIỂN THỊ','unverified':'CHƯA XÁC MINH'}[s]||s);
  ...
  function blankProject(boardId){
    return {schemaVersion:1,projectId:uid('project'),name:'Dự án phòng Arduino',targetBoardId:boardId,policy:'strict',systemState:'READY',components:[
  ...
  function renderBoard(){
    ...
    el.innerHTML=`<div class="board-card"><div class="board-head"><div><span class="lab-kicker">${esc(viStatus(app.board.verificationStatus))}</span><b>${esc(app.board.name)}</b><small>${app.board.logicVoltage}V logic · Sơ đồ chân có nguồn xác minh</small></div><div class="board-meta">${app.board.pins.length} cổng đã công bố</div></div><div class="pin-grid"><div class="pin-bank"><div class="pin-bank-title">GPIO / TƯƠNG TỰ</div>${left.map(p=>pin(p,'right')).join('')}</div><div class="pin-bank"><div class="pin-bank-title">GPIO / BUS</div>${right.map(p=>pin(p,'left')).join('')}</div><div class="pin-bank"><div class="pin-bank-title">NGUỒN</div>${power.map(p=>pin(p,'right')).join('')}</div></div><div style="padding:0 12px 12px;color:#8fa1b8;font-size:8px">Nguồn: <a href="${esc(app.board.sourceRef)}" target="_blank" rel="noreferrer" style="color:#b9c8db">tài liệu chính thức</a></div></div>`;
  ...
  function renderGenericNode(c){
    ...
    el.innerHTML=`<div class="component-head"><span class="component-type">${esc(viSupport(c.supportLevel||'unverified'))}</span><b>${esc(c.name)}</b><button class="node-x" data-remove="${esc(c.id)}">×</button></div><div class="component-body"><div class="led-glyph">◈</div><small>${esc(viCategory(c.category))} · ${esc(viStatus(c.verificationStatus||'unverified'))}</small><div class="port-list"><span class="port"><span class="port-label">Chưa có cổng được xác minh</span><i></i></span></div></div>`;
  ...
  function renderAll(){
    renderBoard(); app.project.components.filter(c=>c.kind==='visual').forEach(renderGenericNode); renderNodeClasses(); syncPositions(); enableNodeDrag(); enablePortEvents(); renderWireLayer(); renderConsole(); renderEvents(); updateStats(); updateLesson(); updateState(); updateArtifact();
  }
  ...
  function updateState(){
    ...
    $('#simState').textContent=viStatus(app.project.systemState);
  }
  ...
  function updateArtifact(){const a=app.project.artifact;$('#artifactState').textContent=a&&!app.project.artifactStale?'HỢP LỆ':'CŨ';$('#artifactState').className='status-chip '+(a&&!app.project.artifactStale?'ok':'warning');}
  function updateBoardBadge(){const b=app.board;const e=$('#boardVerification');e.textContent=b?viStatus(b.verificationStatus):viStatus('unverified');e.className='status-chip '+(b?.verificationStatus==='verified'?'ok':'warning');}
  ...
  function renderConsole(){
    ...
    body.innerHTML=ds.length?ds.map(d=>diagCard(d)).join(''):`<div class="empty-console"><div><strong>Console đang sạch</strong><span>Mọi diagnostic vẫn được giữ trong lịch sử dự án; Xóa chỉ dọn phần hiển thị hiện tại.</span></div></div>`;
  }
  function diagCard(d){const resolved=d.resolved?' · Đã giải quyết':'';return `<article class="console-card ${esc(d.severity)}" data-diag="${esc(d.diagnosticId)}"><div class="cc-top"><span class="sev ${esc(d.severity)}">${esc(viStatus(d.severity))}</span><span class="cc-code">${esc(d.errorCode)}${esc(resolved.replace('Đã giải quyết','Đã giải quyết'))}</span></div><h4>${esc(d.studentMessage)}</h4><p>${esc(d.actual||'')} ${d.expected?'· mong đợi: '+esc(d.expected):''}</p><div class="cc-meta"><span class="cc-tag">${esc(d.subsystem)}</span><span class="cc-tag">×${d.occurrenceCount||1}</span>${d.runBlocked?'<span class="cc-tag">CHẶN CHẠY</span>':''}</div><div class="cc-actions"><button data-action="focus" data-id="${esc(d.diagnosticId)}">Định vị</button><button data-action="correct" data-id="${esc(d.diagnosticId)}">Chân đúng</button><button data-action="why" data-id="${esc(d.diagnosticId)}">Tại sao?</button><button data-action="fix" data-id="${esc(d.diagnosticId)}">Cách sửa</button><button data-action="copy" data-id="${esc(d.diagnosticId)}">Sao chép</button></div></article>`;}
  function renderEvents(){const body=$('#eventsBody'); if(!body)return;body.innerHTML=(app.project?.events||[]).slice(0,120).map(e=>`<div class="event-row"><b>${esc(e.errorCode||e.subsystem)} — ${esc(e.message)}</b><small>${new Date(e.timestamp).toLocaleString('vi-VN')} · ${esc(viSubsystem(e.subsystem))} · ${esc(viStatus(e.severity))}</small></div>`).join('')||'<div class="empty-console"><div><strong>Chưa có event</strong><span>Thao tác wiring, compile, runtime và recovery sẽ xuất hiện ở đây.</span></div></div>';}

  function openDiagnostic(d,whyMode=false){app.activeDiagnostic=d;$('#modalTitle').textContent=d.errorCode||'CHẨN ĐOÁN';$('#modalBody').innerHTML=diagnosticDetail(d,whyMode);$('#diagnosticModal').classList.add('open');$('#diagnosticModal').setAttribute('aria-hidden','false');focusDiagnostic(d);}
  function closeDiagnostic(){$('#diagnosticModal').classList.remove('open');$('#diagnosticModal').setAttribute('aria-hidden','true');}
  function diagnosticDetail(d,why=false){return `<div class="diag-detail"><div class="diag-row"><b>Học sinh</b><span>${esc(d.studentMessage)}</span></div><div class="diag-row"><b>Phân hệ</b><span>${esc(d.subsystem)}</span></div><div class="diag-row"><b>Nguồn / đích</b><span>${esc(d.portIds?.join(' → ')||'—')}</span></div><div class="diag-row"><b>Thực tế</b><span>${esc(d.actual||'—')}</span></div><div class="diag-row"><b>Mong đợi</b><span>${esc(d.expected||'—')}</span></div><div class="diag-box"><strong>${why?'Tại sao?':'Nguyên nhân'}</strong><p>${esc(d.cause||d.technicalMessage||'Không có mô tả.')}</p></div><div class="diag-box"><strong>Cách sửa</strong><p>${esc(d.suggestedFix||'Kiểm tra sơ đồ chân và bảng tương thích rồi nối lại.')}</p></div><div class="diag-row"><b>Trạng thái</b><span>${d.resolved?'Đã giải quyết':'Đang tồn tại'}${d.runBlocked?' · Chạy bị chặn trong chế độ nghiêm ngặt':''}</span></div></div>`;}
  ...
  function copyDiagnostic(d){const text=`${d.errorCode}\n${d.studentMessage}\nThực tế: ${d.actual||'-'}\nMong đợi: ${d.expected||'-'}\nNguyên nhân: ${d.cause||'-'}\nCách sửa: ${d.suggestedFix||'-'}`;navigator.clipboard?.writeText(text).then(()=>toast('Đã sao chép thông tin chẩn đoán.')).catch(()=>toast('Clipboard không khả dụng.','warning'));}
  ...
  function compile(){
    ...
    for(const ref of [...new Set(pinRefs)]){const id=/^A\d+$/.test(ref)?ref:'D'+ref;const pin=app.board?.pins.find(p=>p.id===id);if(!pin){errs.push({code:'FW001',msg:`Firmware đang tham chiếu ${ref} nhưng cấu hình bo mạch không có chân này.`});continue;}const pm=new RegExp(`pinMode\\s*\\(\\s*${ref}\\s*,\\s*OUTPUT\\s*\\)`,'i').test(code);if(pm&&pin.inputOnly)errs.push({code:'GPIO002',msg:`${id} chỉ dùng làm đầu vào nhưng firmware đặt OUTPUT.`});const other=app.project.wires.filter(w=>portKey(w.source.componentId,w.source.portId)===`board-1:${id}`||portKey(w.target.componentId,w.target.portId)===`board-1:${id}`);if(!other.length)errs.push({code:'FW001',msg:`Firmware dùng ${ref} nhưng chân chưa có kết nối trong sơ đồ mạch.`});}
    if(errs.length){errs.forEach(e=>log('error',e.code,'Compiler',e.msg,{technicalMessage:e.msg,cause:'Kiểm tra bản đồ tài nguyên firmware / trình phân tích mã.',suggestedFix:'Sửa mã hoặc nối đúng sơ đồ; hệ thống không tự sửa mã.',runBlocked:true}));app.project.systemState='FAULT';updateState();renderAll();return false;}
    const warnings=app.project.components.filter(c=>c.kind!=='board').filter(c=>c.kind!=='visual').length;
    if(app.board?.verificationStatus!=='verified')log('warning','ASSET001','Asset','Cấu hình bo mạch chưa được xác minh.');
    app.project.artifact={artifactId:uid('artifact'),targetBoardId:app.board.boardId,toolchain:'browser-compiler-lite',createdAt:now(),codeHash:btoa(unescape(encodeURIComponent(code))).slice(0,24),diagnostics:[],stale:false};app.project.artifactStale=false;app.project.systemState='STOPPED';pushEvent('Compiler','COMP_OK','Biên dịch thành công bằng trình biên dịch nhẹ trên trình duyệt','success',{targetBoardId:app.board.boardId});persist();renderAll();toast('Biên dịch thành công — tệp biên dịch hợp lệ.','success');return true;
  }
  ...
  function run(){
    ...
    if(strictBlocking()){const d=app.project.diagnostics.find(x=>!x.resolved&&x.runBlocked);log('error','RUN_BLOCKED','Runtime',`Chạy bị chặn vì ${d?.errorCode||'blocking diagnostic'}.`,{cause:'Chính sách chế độ nghiêm ngặt.',suggestedFix:d?.suggestedFix||'Giải quyết các chẩn đoán đang chặn trước khi Chạy.',runBlocked:true});openDiagnostic(d||app.project.diagnostics[0]);return;}
    app.project.systemState='RUNNING';pushEvent('Runtime','RUN_START','Mô phỏng bắt đầu','success',{targetBoardId:app.board.boardId});serialWrite('--- bắt đầu mô phỏng ---');renderAll(); toast('Mô phỏng đang chạy.','success');simulateFirmware();
  }
  ...
  function stopRun(){clearRuntimeTimers();app.project.systemState='STOPPED';pushEvent('Runtime','RUN_STOP','Mô phỏng đã dừng','info');serialWrite('--- dừng mô phỏng ---');renderAll();}
  function reset(){clearRuntimeTimers();const boardId=app.project.targetBoardId;saveSnapshot(true);app.project=blankProject(boardId);app.activeDiagnostic=null;app.wireDraft=null;app.focusTarget=null;app.correctTarget=null;app.activeWireId=null;$('#codeEditor').value=DEFAULT_CODE;persist();renderAll();toast('Dự án đã trở về trạng thái an toàn.','success');}
  ...
  function finishWire(e){if(!app.wireDraft)return;const source=app.wireDraft.source;const under=document.elementFromPoint(e.clientX,e.clientY)?.closest?.('.port');app.wireDraft=null;$$('.port').forEach(p=>p.classList.remove('invalid-target','correct-target'));if(under)addWire(source,{componentId:under.dataset.component,portId:under.dataset.port});else log('warning','WIRE004','Wiring','Thiếu điểm nối — đầu dây không nằm trên một cổng hợp lệ.',{cause:'Pointer released outside a recognized port.',suggestedFix:'Kéo lại đầu dây và thả đúng vào cổng đích.',runBlocked:false});renderWireLayer();}

  function enableNodeDrag(){
    $$('#ledNode .component-head,#buttonNode .component-head,.board-node .board-head,.component-node[id^="node-"] .component-head').forEach(h=>{
      if(h.dataset.dragBound)return;
      h.dataset.dragBound='1';
      h.addEventListener('pointerdown',e=>{
        if(e.target.closest('.node-x')||e.button!==0)return;
        const el=h.closest('.component-node,.board-node');
        const id=el?.id==='ledNode'?'led-1':el?.id==='buttonNode'?'button-1':el?.id==='boardNode'?'board-1':el?.id?.replace('node-','');
        const c=app.project.components.find(x=>x.id===id);
        if(!c || (c.locked && id!=='board-1'))return;
        e.preventDefault();
        saveSnapshot(true);
        const start={x:e.clientX,y:e.clientY,cx:c.x,cy:c.y};
        const move=ev=>{
          c.x=Math.max(8,start.cx+(ev.clientX-start.x)/app.zoom);
          c.y=Math.max(8,start.cy+(ev.clientY-start.y)/app.zoom);
          positionComponent(c.id);renderWireLayer();
        };
        const up=()=>{
          window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);
          persist();renderAll();toast(`${c.name||app.board?.name||'Linh kiện'} đã được di chuyển.`,'success');
        };
  ...
  function library(){
    ...
    list.innerHTML=ms.map(m=>`<div class="module-card" draggable="true" data-module-id="${esc(m.moduleId)}"><span class="m-icon"><i class="${m.category==='Sensor'?'cil-speedometer':m.category==='Board'?'cil-memory':m.category==='Actuator'?'cil-bolt':'cil-puzzle'}"></i></span><div><b>${esc(m.accessibilityLabels?.name||m.variant||m.moduleId)}</b><small>${esc(viCategory(m.category))} · ${esc(m.moduleId)}</small></div><span class="m-badge ${esc(m.verificationStatus)}">${esc(viStatus(m.verificationStatus))}</span></div>`).join('')||'<div class="empty-console" style="min-height:180px"><div><strong>Không tìm thấy module</strong><span>Đổi từ khóa hoặc category.</span></div></div>';
  ...
  function addModuleToCanvas(moduleId,dropPoint=null){const m=app.modules.find(x=>x.moduleId===moduleId);if(!m)return;const x=dropPoint?Math.max(12,dropPoint.x/app.zoom-105):Math.min(760,120+app.project.components.length*18);const y=dropPoint?Math.max(12,dropPoint.y/app.zoom-28):Math.min(520,120+app.project.components.length*55);const c={id:uid('module'),kind:'visual',name:(m.accessibilityLabels?.name||m.variant||m.moduleId),moduleId:m.moduleId,category:m.category,verificationStatus:m.verificationStatus,supportLevel:m.supportLevel,x,y};saveSnapshot(true);app.project.components.push(c);persist();renderAll();toast(`${m.accessibilityLabels?.name||m.variant||m.moduleId} đã thêm vào màn hình — ${viStatus(m.verificationStatus)}.`,'warning');}
  ...
  function selectBoard(id,resetProject=true){const b=app.boards.find(x=>x.boardId===id)||app.boards[0];if(!b)return;if(resetProject&&app.project&&app.project.targetBoardId!==b.boardId){saveSnapshot(true);app.project=blankProject(b.boardId);}
    app.board=b;app.project.targetBoardId=b.boardId;$('#boardSelect').value=b.boardId;$('#boardVerification').textContent=viStatus(b.verificationStatus);updateBoardBadge();app.project.artifactStale=true;persist();renderAll();pushEvent('Board Registry','BOARD_SELECTED',`Bo mạch đích: ${b.name}`,'info',{targetBoardId:b.boardId});
  }

  function resetCode(){if(confirm('Khôi phục mã mẫu main.ino?')){$('#codeEditor').value=DEFAULT_CODE;app.project.code=DEFAULT_CODE;app.project.artifactStale=true;persist();renderAll();toast('Đã khôi phục code mẫu.');}}
  ...
  function bind(){
    ...
    $('#serialClear').addEventListener('click',()=>$('#serialOutput').textContent='');$('#serialPause').addEventListener('click',()=>{app.serialPaused=!app.serialPaused;$('#serialPause').textContent=app.serialPaused?'Tiếp tục':'Tự cuộn';});
  ...
  function removeComponent(id){if(id==='board-1')return;saveSnapshot(true);const c=app.project.components.find(x=>x.id===id);if(!c)return;app.project.components=app.project.components.filter(x=>x.id!==id);app.project.wires=app.project.wires.filter(w=>w.source.componentId!==id&&w.target.componentId!==id);persist();renderAll();toast(`${c.name||c.componentType||'Linh kiện'} đã được xóa.`);}
  ...
  async function boot(){
    try{
      ...
      $('#boardSelect').innerHTML=app.boards.map(b=>`<option value="${esc(b.boardId)}">${esc(b.name)} · ${esc(viStatus(b.verificationStatus))}</option>`).join('');$('#boardSelect').value=app.board.boardId;$('#policySelect').value=app.project.policy||'strict';updateBoardBadge();
      const cats=['All',...new Set(app.modules.map(m=>m.category))];$('#moduleFilters').innerHTML=cats.map(c=>`<button class="lab-filter ${c==='All'?'active':''}" data-filter="${esc(c)}">${esc(viCategory(c))}</button>`).join('');$$('.lab-filter').forEach(f=>f.addEventListener('click',()=>{$$('.lab-filter').forEach(x=>x.classList.remove('active'));f.classList.add('active');app.moduleFilter=f.dataset.filter;library();}));
